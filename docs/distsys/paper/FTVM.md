...
![center](/distsys/ftvm/ftpro.png){ style="display: block; margin-left: auto; margin-right: auto;" }
...
为了防止出现脑裂的情况，使用共享存储作为协调机制，需在共享存储上执行一个原子性的检测并设置（test-and-set）操作。
成功则允许，失败则说明有另一虚拟机在运行，当前虚拟机会自杀终止。
...
![center](/distsys/ftvm/logchannel.png){ style="display: block; margin-left: auto; margin-right: auto;" }
...
- 集群化陷阱与中断合并：
    当 VM 以较高吞吐率收发数据时，管理程序将多个数据包批量处理——对一组包仅触发一次传输 trap 或接收中断，显著减少上下文切换和中断开销，在理想情况下甚至可实现“零 trap”。
      
- 低延迟日志确认路径：
  为加速主备之间的日志确认、从而减少发送延迟，FT 利用 vSphere 管理程序的延迟执行上下文（类似 Linux tasklet）：
...
![center](/distsys/ftvm/shareddisk.png){ style="display: block; margin-left: auto; margin-right: auto;" }


VMware FT 支持两种磁盘架构以适应不同部署需求：

- 共享磁盘模式（默认）：
主备 VM 共用同一虚拟磁盘，故障切换后数据天然一致。
磁盘写入被视为“对外输出”，仅由主 VM 执行且必须延迟，直到备份 VM 确认日志条目（遵循输出规则）。
- 非共享磁盘模式（用于无共享存储或长距离容错）：
主备各自拥有独立磁盘副本，备份 VM 重放所有磁盘写入以保持同步。
磁盘被视为内部状态，因此主 VM 的写入无需延迟，提升性能。
缺点：初始需显式同步双份磁盘；故障后重启时也需重新同步磁盘状态；FT VMotion 必须同时同步内存状态和磁盘内容。
- 脑裂防护机制（非共享场景下）：
依赖外部仲裁（如第三方服务）或集群多数派（majority）算法决定哪个 VM 可上线，确保只有一个活跃实例，避免数据分裂。
